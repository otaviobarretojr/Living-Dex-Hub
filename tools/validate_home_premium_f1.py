...
'f3 marker':'living-dex-pokemon-detail" content="F3-detail-2.0"' in html,
'load order':html.find('src="game-context-v8.js"')<html.find('src="game-selector-v8.js"')<html.find('src="home-v8.js"')<html.find('src="pokemon-detail-v8.js"'),
...
'f3 audit':"version:'8.0-f3.0'" in detail and 'ld8f3Audit' in detail,
'f3 context inheritance':'gameId()' in detail and 'currentGame:gameId()' in detail and 'primaryBoxContextUntouched:true' in detail,
'f3 tabs':all(x in detail for x in ['Sobre','Evolução','Dados','ld8f3EvolutionPane','ld8f3DataPane']),
'f3 box action preserved':'boxActionPreserved' in detail and '#ld75BoxBtn' in detail,
'f3 evolution preserved':'ld76Evolution' in detail and 'evolutionPreserved' in detail,
'f3 close preserves box':'oldClose=window.closeModal' in detail and "classList.remove('ld8f3-detail-open')" in detail,
'f3 observations hidden':'legacyObservationsHidden' in detail,
'f3 fullscreen':'height:100dvh' in detail_css and '#modal.ld8f3-modal.show{display:flex!important}' in detail_css,
'f3 nav hidden only detail':'body.ld8f3-detail-open .mnav.ld79-nav' in detail_css,
'f3 local image reuse':'.poke-hero img' in detail_css,
}
failed=[k for k,v in checks.items() if not v]
for k,v in checks.items():print(('OK  ' if v else 'FAIL')+k)
if failed:raise SystemExit('F3 validation failed: '+', '.join(failed))
print(f'PHASE F3 POKEMON DETAIL 2.0: {len(checks)}/{len(checks)} checks OK')
