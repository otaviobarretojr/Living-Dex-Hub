...
checks={
 ...
 'mobile safe area':'env(safe-area-inset-bottom)' in css,
...
if failed:raise SystemExit('F1 validation failed: '+', '.join(failed))
print(f'PHASE F1 HOME PREMIUM: {len(checks)}/{len(checks)} checks OK')
