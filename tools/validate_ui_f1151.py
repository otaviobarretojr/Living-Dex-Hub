...
assert "version:'8.0-f11.5.2'" in js
for marker in ['persistentThirdDock:true','legacyDockClassPreserved:true','livingDexClickFunctional:true','directRouteFallback:true','homeDeactivationFallback:true','globalActivationFallback:true','activeDockNormalized:true','androidSafeHeader:true','compactBoxAction:true','detailSourceReadable:true','mutationRecovery:true']:
    assert marker in js, marker
assert 'repeat(3,minmax(0,1fr))' in js
assert "b.className='ld79-extra'" in js
assert 'directLivingDexRoute()' in js
assert "home.style.display='none'" in js
assert "global.style.display='block'" in js
...
print('UI F11.5.2 VALIDATION: PASS • functional Living Dex click • direct routing fallback • persistent 3-item dock • Android safe detail')
