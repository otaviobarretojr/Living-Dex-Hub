...
function nav221DexAction(){
 ...
 return nodes.find(el=>{
   ...
   return /abrir\s+(?:a\s+)?(?:pok[eé]dex|dex)|ver\s+(?:a\s+)?(?:pok[eé]dex|dex)|(?:pok[eé]dex|dex)\s+do\s+jogo/i.test(t);
 });
}
...
function nav221OpenGameDex(id){
 ...
 const press=()=>{
   ...
   // Important: never hide the library/current view when the Dex action was not found.
   document.body.classList.remove('game-dex-focus');
   document.body.dataset.gameDexOpen='0';
...
// Override the 2.1 flow with the guarded 2.2.1 implementation.
...
// Dedicated CI regression: actually attempt to open Scarlet/Violet Dex, not only grep markers.
