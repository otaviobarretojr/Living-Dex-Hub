...
global.document={documentElement:{style:{}},body:{style:{}},getElementById:()=>null,querySelector:()=>null,addEventListener:()=>{}};
...
(async()=>{
  // Business-event journey: add -> no-op -> remove must emit exactly two persisted changes.
  ...
  const changes=[];addEventListener('ld:box-entry-changed',e=>changes.push(e.detail));
  run('collection-reliability-v8.js');
  ...
  // Context journey: primary must stay primary while a different Box runtime is consulted.
  listeners.clear();localStorage.clear();state={caught:{},games:{},activeGameId:'sv'};global.state=state;
  localStorage.setItem('ld8.primaryGame','sv');
  global.GAMES=[{id:'sv',name:'Scarlet / Violet'},{id:'letsgo',name:"Let's Go Pikachu / Eevee"},{id:'bdsp',name:'BDSP'}];
  global.currentGame=GAMES[0];global.currentDex=[];global.activeSubdex=null;
  global.dexRegistryFor=id=>[{id:id+':main'}];global.loadSubdex=async()=>[{id:1},{id:2}];global.saveState=()=>{};global.persist=()=>{};
  run('game-context-v8.js');
  const ok=await ld813SetBoxGame('letsgo',{open:false});assert.strictEqual(ok,true);assert.strictEqual(ld813PrimaryGameId(),'sv');assert.strictEqual(ld813BoxContextId(),'letsgo');assert.strictEqual(state.activeGameId,'sv','consultation must not overwrite primary state');assert.strictEqual(currentGame.id,'letsgo','runtime must use consulted Box game');
  ...
  console.log('JOURNEY RELIABILITY: PASS • collection add/remove events • no-op dedupe • primary/Box isolation • primary persistence');
