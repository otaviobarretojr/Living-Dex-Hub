...
covered=int(data.get('coveredSpecies',0));assert 100<=covered<=151,covered
missing=data.get('missingSpecies') or [];assert len(missing)==151-covered,(covered,len(missing))
...
print(f'BDSP F8.0 VALIDATION: PASS • Sinnoh={covered}/151 • missing={len(missing)}')
