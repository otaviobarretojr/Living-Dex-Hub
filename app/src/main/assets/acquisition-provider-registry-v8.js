/* Living Dex Hub — F6.10 generic acquisition provider registry */
(()=>{'use strict';
...
function ensureBuiltins(){if(!providers.has('letsgo')&&window.ld8LetsGoEncounters){const src=window.ld8LetsGoEncounters;register('letsgo',{gameId:'letsgo',label:"Let's Go Pikachu / Eevee",versions:['pikachu','eevee'],get:id=>src.get?.(id)||null,byVersion:(id,v)=>src.byVersion?.(id,v)||[],meta:()=>src.data||{},audit:()=>src.audit?.()||{}})}return providers}
...
window.ld8AcquisitionProviders={register,unregister,provider,get,list,currentGameId,resolveForPokemon,ensureBuiltins,audit:()=>{ensureBuiltins();return{version:'8.0-f6.10',genericRegistry:true,providerCount:providers.size,providers:[...providers.keys()],letsGoAdapter:providers.has('letsgo'),futureGameReady:true}}};
