/* Living Dex Hub — F11.0 generic acquisition provider registry + multi-game adapters */
(()=>{'use strict';
...
function ensureBuiltins(){
 ...
 if(!providers.has('sv')&&window.ld8SvEncounters){const src=window.ld8SvEncounters;register('sv',{gameId:'sv',label:'Pokémon Scarlet / Violet',versions:['scarlet','violet'],get:id=>src.get?.(id)||null,byVersion:(id,v)=>src.byVersion?.(id,v)||[],meta:()=>src.data||{},audit:()=>src.audit?.()||{}})}
 if(!providers.has('za')&&window.ld8ZaEncounters){const src=window.ld8ZaEncounters;register('za',{gameId:'za',label:'Pokémon Legends: Z-A',versions:['za'],get:id=>src.get?.(id)||null,byVersion:(id,v)=>src.byVersion?.(id,v)||[],meta:()=>src.data||{},audit:()=>src.audit?.()||{}})}
 return providers
}
...
window.ld8AcquisitionProviders={register,unregister,provider,get,list,currentGameId,resolveForPokemon,ensureBuiltins,audit:()=>{ensureBuiltins();return{version:'8.0-f11.0',genericRegistry:true,providerCount:providers.size,providers:[...providers.keys()],letsGoAdapter:providers.has('letsgo'),swordShieldAdapter:providers.has('swsh'),bdspAdapter:providers.has('bdsp'),legendsArceusAdapter:providers.has('arceus'),scarletVioletAdapter:providers.has('sv'),legendsZaAdapter:providers.has('za'),multiGameReady:true}}};
