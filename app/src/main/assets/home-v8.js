...
function activeId(){const s=stateObj();try{return String(s.activeGameId||currentGame?.id||'sv')}catch(e){return String(s.activeGameId||'sv')}}
...
async function continueGame(){const id=IDS.includes(activeId())?activeId():'sv';try{if(typeof window.ld71Open==='function'){await window.ld71Open();return}}catch(e){}try{if(typeof window.openGame==='function')await window.openGame(id)}catch(e){}try{window.ld71Open?.()}catch(e){}}
function switchGame(){try{if(typeof window.ld72OpenGames==='function'){window.ld72OpenGames();return}}catch(e){}console.warn('Living Dex Hub: seletor de jogo indisponível')}
async function chooseGame(id){if(!IDS.includes(id))return;if(id===activeId()){await continueGame();return}try{if(typeof window.ld73LoadGame==='function'){const ok=await window.ld73LoadGame(id);if(ok){try{window.go?.('home')}catch(e){};render();return}}}catch(e){}switchGame()}
...
window.ld8HomeAudit=function(){const id=activeId(),p=progress(id),root=document.getElementById('ld8Home');return {version:'8.0-f1',phase:'F1',mounted:!!root,activeGame:id,progress:p,gameCards:root?.querySelectorAll('.ld8-game').length||0,hero:!!root?.querySelector('.ld8-hero'),continueAction:!!root?.querySelector('[data-ld8="continue"]'),switchAction:!!root?.querySelector('[data-ld8="switch"]'),legacyHomeChildrenVisible:[...document.querySelectorAll('#home > :not(#ld8Home)')].filter(e=>getComputedStyle(e).display!=='none').length}};
