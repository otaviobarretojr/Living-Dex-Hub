/* Living Dex Hub — F2.1 primary game vs consultation context, interaction-safe */
(()=>{
'use strict';
...
async function consultBox(id,{open=true}={}){
 ...
 if(open){try{if(typeof originalOpen==='function')await originalOpen();else window.openGame?.(id)}catch(e){console.error('Living Dex Hub Box open failed',e);return false}}
...
async function selectPrimary(id){
 ...
 /* Commit the explicit user choice first. Runtime loading is secondary and must never undo it. */
...
restorePrimary();
window.ld813Audit=function(){return {version:'8.0-f2.1',primaryGame:primaryId(),boxContext:boxId(),runtimeGame:String(window.currentGame?.id||''),stateActive:String(st().activeGameId||''),selectorMode:selectorMode||null,primaryCommittedFirst:true,boxOpenHook:typeof window.ld71Open==='function',primarySetter:typeof window.ld813SetPrimaryGame==='function',consultHook:typeof window.ld813ConsultGame==='function',detailContextUsesRuntime:true}};
})();
